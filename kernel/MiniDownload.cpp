...
static LONGLONG atoll(const char *p)
{
	LONGLONG n;
	int c, neg = 0;
...
void CMiniDownload::OnGetFileInfo()
{
	...
	*pUrl = 0;
	...
	wcscpy(m_szSavePath, m_pInit);
	wcscat(m_szSavePath, L"\\");
	//
	...
	//url½âÎöÊ§°Ü;
	if (FALSE == InternetCrackUrlW(pUrl, wcslen(pUrl), ICU_ESCAPE, &url))
	{
		response.dwStatu = MNDD_STATU_ANALYSE_URL_FAILED;
		goto ret;
	}
	//Get File Name
	p = m_szUrlPath + wcslen(m_szUrlPath) - 1;
	while (p > m_szUrlPath && p[0] != L'\\' && p[0] != L'/')
		p--;
	if ((p[0] == L'\\' || p[0] == L'/') && p[1])
		wcscat(m_szSavePath, p + 1);
	else
		wsprintfW(m_szSavePath + wcslen(m_szSavePath), L"%u.dat", GetTickCount());
	//
	wcscat(m_szUrlPath, m_szExtraInfo);
	//
	...
	case INTERNET_SCHEME_HTTP:
		...
		if(FALSE == HttpSendRequest(m_hRemoteFile, NULL, NULL, NULL, NULL))
		{
	...
	m_hLocalFile = CreateFileW(m_szSavePath, GENERIC_WRITE, FILE_SHARE_READ, 0, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
	if (m_hLocalFile == INVALID_HANDLE_VALUE)
	{
