...
void CFileManager::OnUploadFromDisk(char*Buffer)
{
	...
	CMiniFileTrans::CMiniFileTransInit*pInit = (CMiniFileTrans::CMiniFileTransInit*)malloc(sizeof(DWORD) + sizeof(WCHAR)*(wcslen(m_pCurDir) + 1 + wcslen((WCHAR*)Buffer) + 1));
...
void CFileManager::OnDownload(char*buffer)
{
	...
	CMiniFileTrans::CMiniFileTransInit*pInit = (CMiniFileTrans::CMiniFileTransInit*)malloc(sizeof(DWORD) + sizeof(WCHAR)*(wcslen((WCHAR*)buffer) + 1));
...
void CFileManager::OnRunFile(DWORD Event,char*buffer)
{
	...
	while (pIt[0])
	{
		...
		if (pIt[0])
		{
			...
			ShellExecuteW(NULL, L"open", pFile, NULL, m_pCurDir, dwShow);
