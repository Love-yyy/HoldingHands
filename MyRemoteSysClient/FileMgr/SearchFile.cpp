...
CSearchFile::CSearchFile(DWORD ThreadCount)
{
	//初始化栈
	m_StackSize = 0;
	m_DirStack = (LPWSTR*)malloc(sizeof(LPWSTR)* MAX_SIZE);
...
void CSearchFile::WorkerThread(SEARCH_THREAD_CONTEXT*pSearchThreadContext)
{
	//
	MSG msg;
	...
	while (GetMessage(&msg, 0, 0, 0))
	{
		...
		CurDirNameLen = wcslen(pCurDir);
		pFirstFileName = (WCHAR*)malloc(sizeof(WCHAR)* (CurDirNameLen + 3));
		wcscpy(pFirstFileName, pCurDir);
		...
		*(pFirstFileName + CurDirNameLen + 2) = 0;
		...
		//Begin trave dir;
		WIN32_FIND_DATA fd;
		HANDLE hFirst = FindFirstFile(pFirstFileName, &fd);
		BOOL bFindNextRet = TRUE;
		...
		while (hFirst != INVALID_HANDLE_VALUE && bFindNextRet)
		{
			...
			if (!(fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) || (wcscmp(fd.cFileName, L".") && wcscmp(fd.cFileName, L"..")))
			{
				...
				{
					NameLen = wcslen(fd.cFileName);

					pTemp = (WCHAR*)malloc(sizeof(WCHAR) * (CurDirNameLen + 1 + NameLen + 1));

					wcscpy(pTemp, pCurDir);
					*(pTemp + CurDirNameLen) = L'\\';
					wcscpy(pTemp + CurDirNameLen + 1, fd.cFileName);
					
					pThis->Push(pTemp);
				}
				DEST_FILE_INFO*pDestFile = &pThis->m_DestFile;
				//找到文件
				if (wcsstr(fd.cFileName, pDestFile->FileName) && (!pDestFile->FileAttrib || /*找到的文件的具备所有目标文件的属性*/((pDestFile->FileAttrib&fd.dwFileAttributes) == pDestFile->FileAttrib)))
				{
...
void CSearchFile::MasterThread(CSearchFile*pThis)
{
	...
	for (DWORD i = 0; i < pThis->m_dwThreadCount; i++)
	{
		...
		SearchThreadContext[i].hThread = CreateThread(0,
			0, 
			(LPTHREAD_START_ROUTINE)CSearchFile::WorkerThread,
	...
	BOOL bPost;
	MSG msg;
	int EndThreadCount = 0;				//记录结束线程的个数
	...
	while (GetMessage(&msg, NULL, NULL, NULL))
	{
		...
		case SEARCH_START_SEARCH_FILE:
			{
				 //第一个目录入栈
				int StartDirLen = wcslen(pThis->m_DestFile.StartDir);
				LPWSTR StartDir = (WCHAR*)malloc(sizeof(WCHAR) * (StartDirLen + 1));

				wcscpy(StartDir, pThis->m_DestFile.StartDir);
				pThis->Push(StartDir);
...
//Search函数只能在一个线程中调用;
int CSearchFile::Search(LPWSTR DestFileName, LPWSTR StartDir, DWORD FileAttribs, pSearchCallFunc pfun1, pSearchEndCall pfun2, LPVOID CallFucnParam)
{
	//正在查找文件
	...
	//设置初始信息
	wcscpy(m_DestFile.FileName, DestFileName);
	wcscpy(m_DestFile.StartDir, StartDir);
